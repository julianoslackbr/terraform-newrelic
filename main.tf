...
module "pod-nginx-sample-restart" {
  ...
  nr_policy       = module.channel.nr_alert_policy_id
	...
	value_function  = "single_value"
	violation_time_limit_seconds =	3600
	nqrl_query = "FROM K8sContainerSample SELECT max(restartCount) - min(restartCount) AS 'Restarts' WHERE clusterName = '${var.cluster_name}-${var.environment}' AND containerName LIKE '%nginx-sample-pod%' FACET clusterName, podName"
	critical = [{ 
        operator              = "above"
        threshold             = "10"
